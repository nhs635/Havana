...
OCTProcess::OCTProcess(const CHavanaDlg* pMainDlg) :
	pMainDlg(pMainDlg), _running(false), discom_value(0),

	raw_size({ pMainDlg->m_Edit_nScans, pMainDlg->m_Edit_nAlines }), raw2_size({ pMainDlg->m_Edit_nScans / 2, pMainDlg->m_Edit_nAlines }),
	fft_size({ pMainDlg->m_Edit_nFFT, pMainDlg->m_Edit_nAlines }), fft2_size({ GET_NEAR_2_POWER(raw2_size.width), pMainDlg->m_Edit_nAlines }),
...
{	
	std::thread calib([&, discom_val, res_show]() {

		// calibration dlg object
		CalibrationDlg* pCalibDlg = pMainDlg->m_VisStream.m_pCalibrationDlg;
...
//		Ipp32f th = CALIBRATION_THRESH;
		...
		for (int ch = 0; ch < 2; ch++)
		{
			...
			CString title; title.Format(_T("FFT of D%d Signal"), ch+1);
			pCalibDlg->DrawGraph(&res2(0, ch), title);
			
			// 2. Maximum detection & Non-peak signal elimination		
			int start, end;
			pCalibDlg->WaitForRange(start, end);
			
			np::Array<float> mask(fft_size.width);
			ippsZero_32f(mask, fft_size.width);
			ippsSet_32f(1.0f, &mask(start), end - start + 1);
		...
		float* temp1 = new float[raw2_size.width];
		for (int i = 0; i < raw2_size.width; i++)
		{
		...
		//CScope scope;
		//scope(temp1, raw2_size.width, -10, 10);
...
void OCTProcess::LoadCalibration(int discom_val, CString bg_path, CString calib_path)
{
	DWORD dwRead, dwTotalRead = 0;
	
	// create file (background)
	HANDLE hBgFile = INVALID_HANDLE_VALUE;
	np::Array<uint16_t, 2> frame(pMainDlg->m_Edit_nScans, pMainDlg->m_Edit_nAlines);
	...
	{
		// calib_index
		ReadFile(hCalibFile, calib_index.raw_ptr(), sizeof(float) * raw2_size.width, &dwRead, NULL);
		dwTotalRead += dwRead;
		...
		discom_value = discom_val;
		ChangeDiscomValue(discom_value);
