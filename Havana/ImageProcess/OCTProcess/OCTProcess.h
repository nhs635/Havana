...
{
public:
	OCTProcess(const CHavanaDlg* pMainDlg);
	~OCTProcess();
	...
	void GenerateCalibration(int discom_val, BOOL res_show);
	void ChangeDiscomValue(int discom_val = 0);
	void SaveCalibration();
	void LoadCalibration(int discom_val = 0, CString bg_path = _T("bg.bin"), CString calib_path = _T("calibration.dat"));
