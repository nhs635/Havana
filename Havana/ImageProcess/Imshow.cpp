...
Imshow::Imshow(CWnd* pParent /*=NULL*/)
	...
	m_pBitmap(nullptr), m_pImageData(nullptr), isRGB(false)
{
}
...
void Imshow::operator() (uint8_t* pImageData, int width, int height, bool rgb) //, float cMin, float cMax)
{
	isRGB = rgb;
	...
	if (!isRGB)
	{
		...
		m_pBitmap = new MyBitmap;
		m_pBitmap->SetBitmap(height, width, 1, 0);
		memcpy(m_pBitmap->GetPtr(), pImageData, sizeof(uint8_t) * m_nImageSize);
		//m_pBitmap->ScaleImage(m_pImageData, m_cMin, m_cMax);
	}
	else
	{
		m_nImageSize = 3 * width * height;
		m_pBitmap = new MyBitmap;
		m_pBitmap->SetBitmap(height, width, 3);
		memcpy(m_pBitmap->GetPtr(), pImageData, sizeof(uint8_t) * m_nImageSize);
	}
...
void Imshow::OnPaint()
{
	...
	if (!isRGB)
	{
		...
		dImshow.StretchBlt(RectWnd.left, RectWnd.top, RectWnd.Width(), RectWnd.Height(),
			&dcMem, 0, 0, RectWnd.Width(), RectWnd.Height(), SRCCOPY);
		//dImshow.StretchBlt(m_ImshowRect.left, m_ImshowRect.top, m_ImshowRect.Width(), m_ImshowRect.Height(),
		//	&dImshow, m_ImshowRect.left, m_ImshowRect.top, m_ImshowRect.Width(), m_ImshowRect.Height(), SRCCOPY);
