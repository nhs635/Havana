...
//const int DATASOURCE_VOLTAGE_RANGE = PX14VOLTRNG_3_487_VPP; // 1.388
...
SignatecDAQ::~SignatecDAQ()
{
	if (_thread.joinable())
	{
		//AfxMessageBox(_T("Stop Acquisition"));
		_running = false;
		_thread.join();
	}
...
{
	int result;
	char* pPreamble = "Failed to initialize PX14400 device: ";
	...
	Sleep(500);
	//BootBufCfgSetPX14(_board, 0, nChannels * nScans * nAlines * 2);
	//BootBufCfgSetPX14(_board, 1, nChannels * nScans * nAlines * 2);
	result = BootBufCheckOutPX14(_board, 1, &dma_bufp, NULL);
...
bool SignatecDAQ::startAcquisition()
{
	...
	SendStatusMessage((CString)("Data acquisition thread is started."));
...
void SignatecDAQ::stopAcquisition()
{
	...
	SendStatusMessage((CString)("Data acquisition thread is finished normally."));
}
...
void SignatecDAQ::run()
{
	...
	{		
		...
		if ((loop_counter % 8 == 0 || loop_counter % 8 == 4) && loop_counter != 0)
		{
			...
			//int frame_length = nScans * nAlines;
			//int step = nChannels * nScans;
			//np::Array<uint16_t, 2> frame2(step, nAlines);
			//ippiCopy_16u_C1R(frame.raw_ptr(), sizeof(uint16_t) * 2 * step, frame2.raw_ptr(), sizeof(uint16_t) * step, { step, nAlines });
		...
		if (dwTickNow - dwTickLastUpdate > 5000)
		{
			...
			if (dwElapsed)
			{
				...
				printf("[Elapsed Time] %u:%02u:%02u [DAQ Rate] %3.2f MS/s [Acquired Frames] %d frames \n", h, m, s, dRate, frameIndex);
			}
		}
	}
...
void SignatecDAQ::dumpError(int res, LPCSTR pPreamble)
{
	...
	if ((SIG_SUCCESS == my_res) && pErr)	
		str = ((CString)pPreamble + (CString)((LPCSTR)pErr)); // Error Message	
	else	
		str = ((CString)pPreamble + (CString)_T("Cannot obtain error message.."));
	if (res == -6)
		str += (CString)_T(" (Please restart the computer.)");
...
void SignatecDAQ::dumpErrorSystem(int res, LPCSTR pPreamble)
{
	char *pErr = nullptr;

	CString str; str.Format(_T("Error code (%d)"), res);	
	SendStatusMessage((CString)pPreamble + str); // Error Message	
	AfxMessageBox(str);
